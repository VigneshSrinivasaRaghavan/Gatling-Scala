...
class ReuseMethodsFromBaseTest extends Simulation with BaseTest {
  ...
  val readCreateUpdateDeleteScenario: ScenarioBuilder = scenario("get user")
    ...
    .pause(1)
    .exec(createUser("data/requestData/createUser.json",201))
    .pause(1)
    .exec(updateUser(2, "data/requestData/updateUser.json",200))
    .pause(1)
